...
// pour importer vos modules créez un fichier package.json avec type : "module"
...
readJSON('./data/dragons.json')
    .then( data  => {
        ...
        const oldest = dragons[dragons.length - 1];
...
// relationships
const fileDragon = readJSON('./data/dragons.json');
const relationships = readJSON('./data/relationships.json');

Promise.all([fileDragon, relationships])
    .then(data => {
        const [dragon, relationships] = data;
        // console.log(dragon);
